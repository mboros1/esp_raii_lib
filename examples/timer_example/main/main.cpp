...
extern "C" void app_main(void) {
  ...
  // Example 4: Multiple timers with context in class
  {
    ...
    auto fast_delegate =
        etl::delegate<void()>::create<TimerContext, &TimerContext::callback>(
            fast_ctx);
    auto slow_delegate =
        etl::delegate<void()>::create<TimerContext, &TimerContext::callback>(
            slow_ctx);
    ...
    ESP_LOGI(TAG, "Final counts - Fast: %d, Slow: %d", fast_ctx.count,
             slow_ctx.count);
  ...
  // Example 5: Timer with restart
  {
    ...
    restart_timer.create("restart",
                         etl::delegate<void()>::create(restart_callback), true);
